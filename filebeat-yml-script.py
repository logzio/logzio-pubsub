...
def _add_topic():
    ...
    with open("./data.yml", "r") as data_yml:
    ...
    for publisher_index in amount_of_publishers:
        amount_of_subscribers = range(len(pubsub_data_dictionary["logzio-pubsub"][publisher_index]["subscriptions"]))
        for subscriber_index in amount_of_subscribers:
            print(publisher_index, ",", subscriber_index)
    ...
    # with open(FILEBEAT_CONF_PATH, "r") as filebeat_read:
    #     print(filebeat_read.read())
